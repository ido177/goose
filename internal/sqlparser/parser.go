...
func ParseSQLMigration(r io.Reader, direction Direction, debug bool) (stmts []string, useTx bool, err error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(strings.TrimSpace(line), "--") && strings.Contains(line, "+goose") {
			...
			case annotationUp:
				...
				default:
					return nil, false, fmt.Errorf("duplicate '-- +goose Up' annotations; stateMachine=%d, see https://github.com/ido177/goose#sql-migrations", stateMachine.state)
				}
				continue
			...
			case annotationDown:
				...
				default:
					return nil, false, fmt.Errorf("must start with '-- +goose Up' annotation, stateMachine=%d, see https://github.com/ido177/goose#sql-migrations", stateMachine.state)
				}
				continue
			...
			case annotationStatementBegin:
				...
				default:
					return nil, false, fmt.Errorf("'-- +goose StatementBegin' must be defined after '-- +goose Up' or '-- +goose Down' annotation, stateMachine=%d, see https://github.com/ido177/goose#sql-migrations", stateMachine.state)
				}
				continue
			...
			case annotationStatementEnd:
				...
				default:
					return nil, false, errors.New("'-- +goose StatementEnd' must be defined after '-- +goose StatementBegin', see https://github.com/ido177/goose#sql-migrations")
				}
		...
		default:
			return nil, false, fmt.Errorf("failed to parse migration: unexpected state %d on line %q, see https://github.com/ido177/goose#sql-migrations", stateMachine.state, line)
		}
	...
	switch stateMachine.get() {
	case start:
		return nil, false, errors.New("failed to parse migration: must start with '-- +goose Up' annotation, see https://github.com/ido177/goose#sql-migrations")
